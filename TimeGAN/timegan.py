...
from preprocessing import TimeDataset, batch_generator
from models.architecture.encoders import RNNnet



def train(params, dataset, embedder, generator, supervisor, recovery, discriminator):
    """Training process."""
    ...
    for step in range(1, max_steps + 1):
        ...
        if step % params.print_every == 0:
            print("step: "+ str(step)+ "/"+ str(max_steps)+ ", loss_d: "+ str(np.round(loss_d.item(), 4))+ ", loss_g_u: "+ str(np.round(loss_g_u.item(), 4))+ ", loss_g_v: "+ str(np.round(loss_g_v.item(), 4))+ ", loss_s: "+ str(np.round(np.sqrt(loss_s.item()), 4))+ ", loss_e_t0: "+ str(np.round(np.sqrt(loss_e_t0.item()), 4)))
...
if __name__ == "__main__":
    ...
    params.model_save_path = "models/saved_models/" + params.dataset
    params.seq_len = 24
    ...
    dataset = TimeDataset(params.data_path, params.seq_len)
    input_size = dataset[0].shape[1]
    hidden_size = 24
    num_layers = 3

    embedder = RNNnet(input_size, hidden_size, hidden_size, num_layers).to(params.device)
    recovery = RNNnet(hidden_size, hidden_size, input_size, num_layers).to(params.device)
    generator = RNNnet(input_size, hidden_size, hidden_size, num_layers).to(params.device)
    supervisor = RNNnet(hidden_size, hidden_size, hidden_size, num_layers - 1).to(params.device)
    discriminator = RNNnet(hidden_size, hidden_size, 1, num_layers, activation_fn=None).to(params.device)

    train(params, dataset, embedder, generator, supervisor, recovery, discriminator)


    if params.save_model:
        torch.save(embedder.state_dict(), params.model_save_path + "_embedder.pt")
        torch.save(generator.state_dict(), params.model_save_path + "_generator.pt")
        torch.save(supervisor.state_dict(), params.model_save_path + "_supervisor.pt")
        torch.save(recovery.state_dict(), params.model_save_path + "_recovery.pt")
        torch.save(discriminator.state_dict(), params.model_save_path + "_discriminator.pt")