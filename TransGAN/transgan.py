...
from layers.transformer import TimeTransformer
...
def transgan(ori_data, params):
    
    embedder = TimeTransformer(params.input_size, params.seq_len, params.hidden_size, params.hidden_size).to(params.device)
    recovery = TimeTransformer(params.hidden_size, params.seq_len, params.hidden_size, params.input_size).to(params.device)
    generator = TimeTransformer(params.input_size, params.seq_len, params.hidden_size, params.hidden_size).to(params.device)
    supervisor = TimeTransformer(params.hidden_size, params.seq_len, params.hidden_size, params.hidden_size).to(params.device)
    discriminator = TimeTransformer(params.hidden_size, params.seq_len, params.hidden_size, 1).to(params.device)
    ...
    for step in range(1, params.max_steps + 1):
        ...
        if step % params.print_every == 0:
            torch.save({'step': step, 'model_state_dict': embedder.state_dict(),
                    'optimizer_state_dict': optimizer_er.state_dict()}, params.model_save_path + '_embedder.pth')
    ...
    for step in range(1, params.max_steps + 1):
        ...
        if step % params.print_every == 0:
            torch.save({'step': step, 'model_state_dict': supervisor.state_dict(),
                    'optimizer_state_dict': optimizer_gs.state_dict()}, params.model_save_path + '_supervisor.pth')
    ...
    for step in range(1, params.max_steps + 1):
        ...
        if step % params.print_every == 0:
            ...
            torch.save({'step': step, 'model_state_dict': generator.state_dict(),
                    'optimizer_state_dict': optimizer_gs.state_dict()}, params.model_save_path + '_generator.pth')
            
            torch.save({'step': step, 'model_state_dict': discriminator.state_dict(),
                    'optimizer_state_dict': optimizer_d.state_dict()}, params.model_save_path + '_discriminator.pth')
            
            torch.save({'step': step, 'model_state_dict': recovery.state_dict(),
                    'optimizer_state_dict': optimizer_er.state_dict()}, params.model_save_path + '_recovery.pth')

            logger.log("step: "+ str(step)+ "/"+ str(params.max_steps)+ ", loss_d: "+ str(np.round(loss_d.item(), 4))+ ", loss_g_u: "+ str(np.round(loss_g_u.item(), 4))+ ", loss_g_v: "+ str(np.round(loss_g_v.item(), 4))+ ", loss_s: "+ str(np.round(np.sqrt(loss_s.item()), 4))+ ", loss_e_t0: "+ str(np.round(np.sqrt(loss_e_t0.item()), 4)))
    
    logger.log("Finish Joint Training")
    
    x = data_gen.__next__()
    z = torch.randn(ori_data.shape[0], x.size(1), x.size(2)).to("cpu")
    generator = generator.to("cpu")
    supervisor = supervisor.to("cpu")
    recovery = recovery.to("cpu")
    e_hat = generator(z)
    h_hat = supervisor(e_hat)
    x_hat = recovery(h_hat)
